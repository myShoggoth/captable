...
main = hspec $ do
    ...
    describe "CapTable" $ do
      it "calculates a simple pps" $ do
        let i = Investment angel (Currency 2000000)
        let r = Round "Seed" (fromGregorian 2000 1 1) (PreMoneyValuation 3000000) [i] Common (OptionPool (OptionPoolPercent 0.0) PostMoney) None
        let c = Company [r] [Investment (Investor "Founder 1") (Shares 750000), Investment (Investor "Founder 2") (Shares 750000)]
        let pps' = evalState (pps r) (Calculation c Nothing)
        let ts' = evalState (ts (Just r)) (Calculation c Nothing)
        pps' `shouldBe` 2.00
        ts' `shouldBe` 2500000
      it "calculates pps given a post money option pool" $ do
        let i = Investment angel (Currency 1000000)
        let r = Round "Seed" (fromGregorian 2000 1 1) (PreMoneyValuation 4000000) [i] Common (OptionPool (OptionPoolPercent 0.15) PostMoney) None
      ...
      it "calculates the pre money option pool shares" $ do
        let i = Investment angel (Currency 1000000)
        let r = Round "Seed" (fromGregorian 2000 1 1) (PreMoneyValuation 1000000) [i] Common (OptionPool (OptionPoolPercent 0.2) PreMoney) None
        let c = Company [r] [Investment (Investor "Founder 1") (Shares 500000), Investment (Investor "Founder 2") (Shares 500000)]
        let pps' = evalState (pps r) (Calculation c Nothing)
        let result = evalState (ops (Just r)) (Calculation c Nothing)
