...
import CapTable.Types
    ( Calculation,
      ...
      InvestmentBasis(Shares, Currency),
      Round(Round),
      OptionPool(OptionPool),
      OptionPoolSize(OptionPoolPercent, OptionPoolShares),
      OptionPoolCreation(PreMoney),
      Valuation(PreMoneyValuation, PostEquityStake, PricePerShare) )
import Control.Lens ( (^.) )
...
pps _ (Round _ _ (PricePerShare p) _ _ _ _ _)                             = p
pps c r@(Round _ _ _ _ _ (OptionPool (OptionPoolPercent _) PreMoney) _ _) = (pmv c r) / (fromInteger $ tspmos c r)
...
tspmos c r@(Round _ _ _ _ _ (OptionPool _ PreMoney) _ _) = floor $ prs * (pmo + opp r)
  where
...
pmv _ (Round _ _ (PreMoneyValuation p) _ _ _ _ _) = p
pmv c r@(Round _ _ (PricePerShare p) _ _ _ _ _)   = p * ( fromInteger ( (ts c $ pr c r) + ops c (Just r) ) )
pmv c r@(Round _ _ (PostEquityStake p) _ _ _ _ _) = ( (ti r) / ( ( 1.0 - p ) - fromInteger ( ts c (Just r))) ) * fromInteger ( ts c (pr c r) )

-- | Post-Money Valuation for a round.
postmv :: Calculation -> Round -> Double
postmv _ r@(Round _ _ (PreMoneyValuation p) _ _ _ _ _)  = ti r + p
postmv c r@(Round _ _ (PricePerShare _) _ _ _ _ _)      = ti r + pmv c r
postmv _ (Round _ _ (PostEquityStake _) _ _ _ _ _)      = error "Unable to calculate post-money value due to post-equity stake."
...
ti r@(Round _ _ (PricePerShare p) _ _ _ _ _)   = foldr ((+) . invested (Just p)) 0 (r ^. field @"investments")
...
ops _ Nothing                                                           = 0
ops c (Just r@(Round _ _ _ _ _ (OptionPool (OptionPoolShares 0) _) _ _))  = ops c $ pr c r
ops _ (Just (Round _ _ _ _ _ (OptionPool (OptionPoolShares s) _) _ _))    = s
ops c (Just r@(Round _ _ _ _ _ (OptionPool (OptionPoolPercent 0.0) _) _ _)) = ops c $ pr c r
ops c (Just r@(Round _ _ (PreMoneyValuation _) _ _ (OptionPool (OptionPoolPercent p) PreMoney) _ _)) =
  floor $ ((postmv c r) * p) / pps c r
ops c (Just r@(Round _ _ _ _ _ (OptionPool (OptionPoolPercent p) _) _ _)) = max ops' props
...
opp :: Round -> Double
opp (Round _ _ _ _ _ (OptionPool (OptionPoolPercent p) _) _ _) = p
